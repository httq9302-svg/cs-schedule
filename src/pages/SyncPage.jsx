...
export default function SyncPage() {
  ...
  const doImport = useCallback(async (calId, silent = false) => {
    ...
    try {
      ...
      const events = await fetchEvents(calId, today)
      ...
      if (!silent) {
        ...
        actions.addSyncLog({
          time: now.format('HH:mm'),
          type: 'import',
          msg: `자동 동기화 완료 (${mapped.length}개 일정)`,
          ok: true,
        })
      }
    } catch (e) {
  ...
  useEffect(() => {
    initGoogleAPI()
      .then(async () => {
        ...
        if (alreadySignedIn) {
          // 이미 로그인 상태면 캘린더 목록 불러오기
          try {
            const cals = await listCalendars()
            setCalendars(cals)
            // 저장된 캘린더 ID가 있으면 앱 열 때 자동 가져오기
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          ...
          {signedIn && (
            <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
              <RefreshCw size={11} />
              <span>앱 실행 시 자동 가져오기 · 5분마다 자동 갱신</span>
            </div>
          )}
        </div>
